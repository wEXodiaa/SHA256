...
//                  A RETIRER APRES DEBUGAGE
void binaryPrint(unsigned char array[], int size) {
    for (int count = 0; count < size; count++) {
        for (int i = 7; i >= 0; i--) {
            int bit = (array[count] >> i) & 1;
            printf("%d", bit);
        }
        printf(" ");
...
//                  A RETIRER APRES DEBUGAGE
void binaryPrint32(uint32_t mots[], int nombreDeMots) {
    for (int count = 0; count < nombreDeMots; count++) {
        printf("%2d : ", count);
        for (int bit = 31; bit >= 0; bit--) {
            uint32_t bitValeur = (mots[count] >> bit) & 1;
            printf("%d", bitValeur);
            if (bit % 4 == 0 && bit != 0) {
                printf(" "); // espace tous les 8 bits pour la lisibilité
            }
        }
        printf("\n");
    }
...
int main() {
    ...
    printf("\nResultat final (hash SHA-256 en hex) :\n");
